<template>
  <div class="app-container">
    <el-descriptions title="数据集信息" direction="horizontal" :column="1" border>
      <el-descriptions-item label="数据集名称" label-class-name="my-label" content-class-name="my-content">
        {{ this.dataInfo.name }}
      </el-descriptions-item>
      <el-descriptions-item label="数据集来源">{{ this.dataInfo.purpose }}</el-descriptions-item>
      <el-descriptions-item label="数据集总量">{{ this.dataInfo.count }}</el-descriptions-item>
      <el-descriptions-item label="数据集混淆方式">{{ this.dataInfo.type }}</el-descriptions-item>
      <el-descriptions-item label="数据集详细描述">
      ...
      <!--        <el-tag v-if="!codeInfoTwo.protectInfo.includes('许可')" type="danger">{{ this.codeInfoTwo.protectInfo }}-->
      <!--        </el-tag>-->
      <!--        <el-tag v-if="codeInfoTwo.protectInfo.includes('许可')" type="success">{{ this.codeInfoTwo.protectInfo }}-->
      <!--        </el-tag>-->
      <!--      </el-descriptions-item>-->
...
import { searchDatasetById } from '@/api/self/system'
...
export default {
  ...
  methods: {
    ...
    initGetInfo() {
      searchDatasetById(this.$route.query.id).then(res => {
        this.dataInfo = res
      })
    },
    handleCommitCode() {
